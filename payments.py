...
PROMPT_TEMPLATE = """
You are an AI assistant specialized in configuring financial payment streams. Your job is to generate a well-structured payment stream configuration based on the user's input. Follow these guidelines strictly:
...
def create_or_update_payment_stream(user_input: str, current_config: dict) -> dict:
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a specialized AI assistant for payment stream configurations."},
